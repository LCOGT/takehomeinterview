...
requires boilerplate.html, which contains the bulk of the home page
on first run, generates planets_db.txt to use as a database
between subsequent runs planets_db.txt will be persistent, do not alter its contents
requires node.js
...
// initialize new database file if none exists
if (!fs.existsSync('./planet_db.txt')) {
    fs.writeFile("./planet_db.txt", "", function(err) {
        if(err) throw err;
        console.log("created a new database file: planet_db.txt");
    }); 
}
